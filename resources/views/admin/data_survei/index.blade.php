    ...
    @if ($dataSurvei->count() > 0)
        <div class="survei-grid">
            ...
            <div class="survei-card">
                <!-- Gambar Pratinjau (PAKAI THUMBNAIL DULUAN → SUPER CEPAT!) -->
                <div class="survei-card-image">
                    @if ($item->gambar_thumbnail && Storage::disk('public')->exists($item->gambar_thumbnail))
                        <img src="{{ asset('storage/' . $item->gambar_thumbnail) }}" alt="{{ $item->judul }}" loading="lazy">
                    @elseif ($item->gambar_pratinjau)
                        <img src="{{ asset('storage/' . $item->gambar_pratinjau) }}" alt="{{ $item->judul }}" loading="lazy" style="object-fit: cover;">
                    @else
                        <div class="no-image">
                ...
                <div class="survei-card-content">
                    ...
                    <div class="survei-card-actions">
                        ...
                        <form action="{{ route('admin.data_survei.destroy', $item) }}" method="POST" style="display:inline;">
                            @csrf @method('DELETE')
...
<!-- Loading Overlay & Modal Hapus (tetap ada) -->
