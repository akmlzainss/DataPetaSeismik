    ...
    <div class="filter-bar">
        <form method="GET" action="{{ route('admin.data_survei.index') }}" class="filter-form no-loading">
            <div class="search-box">
                <input type="text" name="search" placeholder="Cari judul, wilayah, tahun..." value="{{ request('search') }}" class="search-input">
            ...
            {{-- Tombol Reset Filter --}}
            @if (request()->hasAny(['search', 'tahun', 'sort']) && (request('search') || request('tahun') || request('sort') != 'terbaru'))
            ...
            <div class="survei-card">
                <div class="survei-card-image">
                    @php
                        ...
                        $imagePath = $item->gambar_thumbnail && Storage::disk('public')->exists($item->gambar_thumbnail) 
                                        ? $item->gambar_thumbnail 
                                        : ($item->gambar_pratinjau && Storage::disk('public')->exists($item->gambar_pratinjau) 
                                            ? $item->gambar_pratinjau 
                                            : null);
                    @endphp

                    @if ($imagePath)
                        <img src="{{ asset('storage/' . $imagePath) }}" alt="{{ $item->judul }}" loading="lazy" style="{{ $item->gambar_thumbnail ? '' : 'object-fit: cover;' }}">
                    @else
                        <div class="no-image">
                ...
                <div class="survei-card-content">
                    ...
                    <div class="survei-card-actions">
                        ...
                        <form action="{{ route('admin.data_survei.destroy', $item) }}" method="POST" style="display:inline;" class="delete-form-trigger">
                            @csrf @method('DELETE')
                            <button type="button" class="btn-delete" title="Hapus" data-judul="{{ $item->judul }}">
    ...
    document.querySelectorAll('.delete-form-trigger button').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (itemTitleElement) {
                ...
                document.querySelector('#deleteModal .modal-message strong').textContent = `data ${title}`;
            }
