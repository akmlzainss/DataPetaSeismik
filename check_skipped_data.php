...
// Script untuk cek data seeder yang di-skip
// Jalankan: php check_skipped_data.php
...
echo "🔍 MENGECEK DATA YANG DI-SKIP DARI SEEDER\n";
echo str_repeat("=", 60) . "\n\n";

// Copy data dari seeder
$dataRack16 = [
...
foreach ($allData as $data) {
    ...
    // Check empty/invalid data
    if (empty($data['daerah']) || $data['daerah'] === '-') {
        $emptyData[] = $data;
        $skip = true;
        $reason = 'Daerah kosong/invalid';
    } elseif (empty($data['ketua_team']) || $data['ketua_team'] === '-') {
        $skipped[] = array_merge($data, ['reason' => 'Ketua team = "-"']);
        $skip = true;
        $reason = 'Ketua team = "-"';
    } elseif (empty($data['tahun']) || $data['tahun'] === '-') {
        $emptyData[] = $data;
        $skip = true;
        $reason = 'Tahun kosong/invalid';
    }
    
    // Check tahun validity
    if (!$skip) {
        $tahunStr = $data['tahun'];
        if (strpos($tahunStr, '/') !== false) {
            $years = explode('/', str_replace(' ', '', $tahunStr));
            $tahun = (int) trim(end($years));
        } else {
            preg_match_all('/\d{4}/', $tahunStr, $matches);
            $tahun = !empty($matches[0]) ? (int) $matches[0][0] : null;
        }
        
        if (!$tahun || $tahun < 1900 || $tahun > 2025) {
            $invalidYear[] = array_merge($data, ['parsed_year' => $tahun]);
        }
    }
}

echo "📋 RINGKASAN:\n";
echo "Total data keseluruhan: " . count($allData) . "\n";
echo "Data di-skip (ketua team '-'): " . count($skipped) . "\n";
echo "Data di-skip (tahun invalid): " . count($invalidYear) . "\n";
echo "Data di-skip (daerah/tahun kosong): " . count($emptyData) . "\n\n";

if (count($skipped) > 0) {
    echo str_repeat("=", 60) . "\n";
    echo "🚫 DATA YANG DI-SKIP KARENA KETUA TEAM = '-':\n";
    echo str_repeat("=", 60) . "\n";
    
    foreach ($skipped as $i => $data) {
        echo "\n" . ($i + 1) . ". {$data['no']} ({$data['tahun']})\n";
        echo "   Daerah: {$data['daerah']}\n";
        echo "   Ketua: {$data['ketua_team']}\n";
    }
}
